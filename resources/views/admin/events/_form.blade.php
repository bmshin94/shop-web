@php
    ...
    $startAtValue = old('start_at', optional($event?->start_at)->format('Y-m-d\TH:i'));
    $endAtValue = old('end_at', optional($event?->end_at)->format('Y-m-d\TH:i'));
@endphp
...
<form action="{{ $formAction }}" method="POST" class="space-y-5">
    ...
    <div class="space-y-2">
        <label class="text-sm font-bold text-text-main">이벤트명</label>
        <input
            type="text"
            name="title"
        ...
        <div class="space-y-2">
            ...
            <select name="status" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 appearance-none outline-none">
                @foreach($statusOptions as $status)
                    <option value="{{ $status }}" {{ old('status', $event?->status ?? '진행예정') === $status ? 'selected' : '' }}>
                        {{ $status }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="space-y-2">
        ...
        <input
            ...
            value="{{ old('banner_image_url', $event?->banner_image_url) }}"
