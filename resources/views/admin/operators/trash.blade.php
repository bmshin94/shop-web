...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h3 class="text-xl font-black text-text-main">삭제된 운영자</h3>
            <p class="mt-1 text-[12px] font-bold text-text-muted">soft delete 처리된 운영자만 표시됩니다.</p>
        </div>
        <a href="{{ route('admin.operators.index') }}" class="inline-flex items-center justify-center gap-1 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-text-main hover:border-primary hover:text-primary transition-all">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('admin.operators.trash') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.operators.trash') }}" class="flex-1 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-text-muted hover:bg-gray-50 transition-colors text-center">
                        초기화
                    </a>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-white/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <p class="text-sm font-extrabold text-text-main">
                휴지통 {{ number_format($operators->total()) }}명
            </p>
        ...
        <div class="hidden lg:grid operator-row px-6 py-4 bg-gray-50/70 border-b border-gray-100">
            <div class="text-[11px] font-bold text-text-muted uppercase">운영자 정보</div>
            <div class="text-center text-[11px] font-bold text-text-muted uppercase">운영자상태</div>
        ...
        <div class="divide-y divide-gray-50">
            @forelse($operators as $operator)
                <div class="operator-row px-4 lg:px-6 py-4 hover:bg-gray-50/60 transition-colors">
                    <div class="min-w-0">
                        <p class="text-sm font-extrabold text-text-main truncate">{{ $operator->name }}</p>
                    ...
                    <div class="text-left lg:text-center">
                        <p class="text-[12px] font-bold text-text-main">{{ optional($operator->deleted_at)->format('Y.m.d H:i') ?: '-' }}</p>
                    </div>
                    <div class="flex lg:justify-center gap-2">
                        <form action="{{ route('admin.operators.restore', $operator) }}" method="POST">
                            @csrf
                            @method('PATCH')
                        ...
                        <form
                            action="{{ route('admin.operators.force-destroy', $operator) }}"
                            method="POST"
                            class="js-confirm-submit"
                            data-confirm-title="운영자 영구 삭제"
                            data-confirm-message="이 운영자를 영구 삭제하시겠습니까? 삭제 후에는 복구할 수 없습니다."
            ...
            @empty
                <div class="px-6 py-20 text-center">
                    ...
                    <p class="text-text-muted text-sm font-bold">
                        {{ $activeFilterCount > 0 ? '조건에 맞는 삭제 운영자가 없습니다.' : '휴지통이 비어 있습니다.' }}
                    </p>
                    <p class="mt-2 text-[12px] font-bold text-text-muted">
                        {{ $activeFilterCount > 0 ? '검색 조건을 조정하거나 초기화 후 다시 확인해 주세요.' : '운영자를 삭제하면 이곳에서 복구 또는 영구삭제할 수 있습니다.' }}
                    </p>
                </div>
            @endforelse
        </div>
