...
<div class="max-w-4xl mx-auto">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" required placeholder="상품 이름을 입력해주세요" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="brief_description" value="{{ old('brief_description', $product->brief_description) }}" placeholder="상품에 대한 간단한 설명을 입력해주세요" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                </div>
                <div class="space-y-2">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="text" id="price" name="price" value="{{ old('price', $product->price) }}" required inputmode="numeric" class="w-full pl-10 pr-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none format-number">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" required inputmode="numeric" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none text-center format-number">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div id="selected-related-products" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="relative p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center gap-3 group animate-fade-in" id="related-item-{{ $rel->id }}">
                            <input type="hidden" name="related_products[]" value="{{ $rel->id }}">
                            <img src="{{ $rel->image_url }}" class="size-12 rounded-xl object-cover bg-white shadow-sm">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-text-main truncate">{{ $rel->name }}</p>
                                <p class="text-[10px] font-bold text-text-muted">ID: {{ $rel->id }}</p>
                            </div>
                            <button type="button" onclick="removeRelatedProduct({{ $rel->id }})" class="p-1 text-text-muted hover:text-red-600 transition-colors">
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between pb-2 border-b border-gray-100">
                    <div class="flex items-center gap-2">
                        ...
                        <h4 class="text-lg font-bold text-text-main">상품 이미지 (최대 5개)</h4></div>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-6">
                    @for($i = 1; $i <= 5; $i++)
                        @php
                            $existingImage = $product->images->where('sort_order', $i - 1)->first();
                        @endphp
                        ...
                        <div id="slot-{{ $i }}" class="upload-slot group {{ $existingImage ? 'has-image' : '' }}" onclick="document.getElementById('file-{{ $i }}').click()">
                            ...
                            <!-- 기존 이미지 출력 -->
                            @if($existingImage)
                                <img id="preview-{{ $i }}" class="preview-img" src="{{ Str::startsWith($existingImage->image_path, 'http') ? $existingImage->image_path : asset($existingImage->image_path) }}">
    ...
    <div class="mt-12 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm space-y-6">
        <div class="flex items-center gap-3 border-b border-gray-50 pb-4">
            <div class="size-10 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center">
                <span class="material-symbols-outlined text-[24px]">edit_note</span>
            </div>
            <div>
                <h4 class="text-lg font-bold text-text-main">상품 수정 가이드</h4>
                <p class="text-[11px] font-bold text-text-muted tracking-tight">안전하고 정확한 상품 정보 업데이트를 위한 가이드입니다.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div class="flex gap-3">
                    <span class="text-amber-600 font-black text-base">01.</span>
                    <div>
                        <h5 class="text-sm font-extrabold text-text-main mb-1">정보 동기화</h5>
                        <p class="text-[11px] font-bold text-text-muted tracking-tight leading-relaxed">상품 정보를 수정하면 사용자 화면에 즉시 반영됩니다. 특히 판매가나 할인율 변경 시 신중하게 확인 후 저장해주세요.</p>
                    </div>
                </div>
                <div class="flex gap-3">
                    <span class="text-amber-600 font-black text-base">02.</span>
                    <div>
                        <h5 class="text-sm font-extrabold text-text-main mb-1">이미지 교체 및 삭제</h5>
                        <p class="text-[11px] font-bold text-text-muted tracking-tight leading-relaxed">기존 이미지를 클릭하여 새로운 파일로 교체할 수 있습니다. 이미지를 삭제하면 서버에서도 실제 파일이 제거되니 주의하세요.</p>
                    </div>
                </div>
            </div>
            <div class="space-y-4">
                <div class="flex gap-3">
                    <span class="text-amber-600 font-black text-base">03.</span>
                    <div>
                        <h5 class="text-sm font-extrabold text-text-main mb-1">품절 및 숨김 처리</h5>
                        <p class="text-[11px] font-bold text-text-muted tracking-tight leading-relaxed">재고가 없거나 일시적으로 판매를 중단해야 할 경우 상태를 '품절' 또는 '숨김'으로 변경하세요. '숨김'은 목록에서 아예 보이지 않게 됩니다.</p>
                    </div>
                </div>
                <div class="flex gap-3">
                    <span class="text-amber-600 font-black text-base">04.</span>
                    <div>
                        <h5 class="text-sm font-extrabold text-text-main mb-1">옵션 변경 주의</h5>
                        <p class="text-[11px] font-bold text-text-muted tracking-tight leading-relaxed">색상이나 사이즈 옵션을 대폭 수정할 경우, 기존 장바구니에 담은 고객의 정보와 차이가 발생할 수 있으니 운영에 참고 바랍니다.</p>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        let selectedProductIds = {!! $product->relatedProducts->pluck('id')->toJson() !!};
