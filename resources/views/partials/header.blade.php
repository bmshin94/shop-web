  ...
  <div class="mx-auto flex h-[88px] max-w-7xl items-center justify-between px-6 lg:px-8">
    ...
    <div class="flex items-center gap-1">
      <a href="/mypage/wishlist"
        ...
        <span class="header-wishlist-count absolute top-0 right-0 {{ auth()->check() && auth()->user() instanceof \App\Models\Member && auth()->user()->wishlists()->count() > 0 ? 'flex' : 'hidden' }} size-[18px] items-center justify-center rounded-full bg-primary text-[9px] leading-none font-bold text-white ring-1 ring-white shadow-sm shrink-0">@if(auth()->check() && auth()->user() instanceof \App\Models\Member){{ auth()->user()->wishlists()->count() }}@else 0 @endif</span>
      </a>
      <a href="{{ route('cart.index') }}"
        ...
        <span class="header-cart-count absolute top-0 right-0 {{ auth()->check() && auth()->user() instanceof \App\Models\Member && auth()->user()->carts()->count() > 0 ? 'flex' : 'hidden' }} size-[18px] items-center justify-center rounded-full bg-primary text-[9px] leading-none font-bold text-white ring-1 ring-white shadow-sm shrink-0">@if(auth()->check() && auth()->user() instanceof \App\Models\Member){{ auth()->user()->carts()->count() }}@else 0 @endif</span>
      </a>
      <a href="/mypage"
