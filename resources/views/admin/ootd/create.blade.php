...
@section('title', 'OOTD 등록 - Active Women 관리자')

@section('content')
<div class="px-8 py-8 max-w-4xl">
    <div class="mb-10">
        <h1 class="text-3xl font-extrabold text-text-main tracking-tight">새 OOTD 등록 </h1>
        <p class="mt-2 text-sm text-text-muted">고객님의 멋진 스타일링 샷을 등록해 주세요.</p>
    </div>

    <form action="{{ route('admin.ootds.store') }}" method="POST" class="space-y-8 bg-white p-10 rounded-3xl border border-gray-100 shadow-sm">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="likes_count" value="0" min="0" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all">
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <textarea name="content" rows="6" placeholder="스타일링에 대한 설명을 입력해 주세요 " class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all"></textarea>
        </div>
