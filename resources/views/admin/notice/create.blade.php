...
@section('title', '공지사항 등록 - Active Women 관리자')

@section('content')
<div class="px-8 py-8 max-w-4xl">
    <div class="mb-10">
        <h1 class="text-3xl font-extrabold text-text-main tracking-tight">새 공지사항 등록 </h1>
        <p class="mt-2 text-sm text-text-muted">중요한 공지사항이나 일반 소식을 등록해 주세요.</p>
    </div>

    <form action="{{ route('admin.notices.store') }}" method="POST" class="space-y-8 bg-white p-10 rounded-3xl border border-gray-100 shadow-sm">
        @csrf
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="title" placeholder="공지사항 제목을 입력해 주세요" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all">
        </div>

        <div class="space-y-2">
            <label class="text-sm font-bold text-text-main">상세 내용</label>
            <textarea name="content" rows="10" placeholder="공지사항 상세 내용을 입력해 주세요 " class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all"></textarea>
        </div>

        <div class="flex items-center gap-10 pt-4">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="hidden" name="is_important" value="0">
                <input type="checkbox" name="is_important" value="1" class="w-5 h-5 text-primary border-gray-300 rounded focus:ring-primary/20">
