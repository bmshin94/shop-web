...
<main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-16">
    ...
    <nav class="flex items-center gap-2 text-sm text-text-muted mb-6">
        ...
        <span class="material-symbols-outlined text-xs">chevron_right</span>
        <span class="font-bold text-text-main">취소/반품/교환 내역</span>
    </nav>

    <!-- Page Title -->
    <h2 class="text-3xl font-extrabold text-text-main tracking-tight mb-8">취소/반품/교환 내역</h2>
                    ...
                    <div class="flex flex-col lg:flex-row gap-4">
                        ...
                        <div class="w-full lg:w-[220px] relative">
                            <select name="status" class="w-full pl-4 pr-10 py-3 bg-gray-50 border border-transparent rounded-2xl text-sm focus:ring-2 focus:ring-primary/20 focus:bg-white focus:border-primary/30 transition-all outline-none font-medium appearance-none cursor-pointer !bg-none">
                                <option value="">모든 신청상태</option>
                                <option value="취소완료" {{ request('status') == '취소완료' ? 'selected' : '' }}>취소완료</option>
                                <option value="반품신청" {{ request('status') == '반품신청' ? 'selected' : '' }}>반품신청</option>
                            ...
                            <tr class="group hover:bg-gray-50/30 transition-colors">
                                ...
                                <td class="py-6 px-6">
                                    <div class="flex items-center gap-5">
                                        ...
                                        <div class="min-w-0">
                                            <a href="{{ route('product-detail', ['slug' => $item->product->slug]) }}" class="text-sm font-semibold text-text-main line-clamp-1 group-hover:text-primary transition-colors">
                                                {{ $item->product_name }}
                                            </a>
                                ...
                                <td class="py-6 px-6 text-center align-top">
                                    <span class="text-xs font-semibold text-text-main tracking-tight">
                                        @if($order->order_status === '취소완료')
                                            주문취소
                                        @elseif($order->payment_status === '환불완료')
                                            반품/환불
                                        @else
                                            기타신청
                                        @endif
                                ...
                                <td class="py-6 px-8 text-center align-top">
                                    <span class="inline-flex py-1.5 px-4 bg-red-50 text-red-600 font-semibold text-[10px] rounded-full border border-red-100 shadow-sm">
                                        {{ $order->order_status }}
                                    </span>
                                </td>
                            </tr>
                                ...
                                <td colspan="4" class="py-32 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div class="size-20 rounded-full bg-gray-50 flex items-center justify-center mb-6">
                                            <span class="material-symbols-outlined text-4xl text-gray-200">assignment_return</span>
                                        </div>
                                        <p class="text-text-muted font-bold">취소/반품/교환 내역이 없습니다.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
