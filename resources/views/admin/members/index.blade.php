...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('admin.members.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.members.index') }}" class="flex-1 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-text-muted hover:bg-gray-50 transition-colors text-center">
                        초기화
                    </a>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 pt-4 border-t border-gray-100">
                <div class="relative flex-1 group">
                    ...
                    <input type="text" name="joined_from" value="{{ request('joined_from') }}" placeholder="가입 시작일" class="datepicker w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                ...
                <div class="relative flex-1 group">
                    ...
                    <input type="text" name="joined_to" value="{{ request('joined_to') }}" placeholder="가입 종료일" class="datepicker w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-white/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <p class="text-sm font-extrabold text-text-main">
                검색 결과 {{ number_format($members->total()) }}명
            </p>
            <div class="flex items-center gap-2 text-[12px] font-bold text-text-muted">
                <a href="{{ route('admin.members.trash') }}" class="inline-flex items-center gap-1 rounded-full border border-gray-200 bg-white px-3 py-1 text-text-main hover:border-primary hover:text-primary transition-colors">
                    <span class="material-symbols-outlined text-[14px]">delete</span>
                    휴지통 {{ number_format($trashedMembersCount ?? 0) }}
                </a>
        ...
        <div class="divide-y divide-gray-50">
            @forelse($members as $member)
                <div class="member-row px-4 lg:px-6 py-4 hover:bg-gray-50/60 transition-colors">
                    <div class="min-w-0">
                        <a href="{{ route('admin.members.show', $member) }}" class="text-sm font-extrabold text-text-main hover:text-primary transition-colors block truncate">
                            {{ $member->name }}
                        </a>
                    ...
                    <div class="text-left lg:text-center">
                        <p class="text-[12px] font-bold text-text-main">{{ optional($member->last_login_at)->format('Y.m.d H:i') ?: '-' }}</p>
                    </div>

                    <div class="flex lg:justify-center">
                        <a href="{{ route('admin.members.show', $member) }}" class="inline-flex items-center justify-center gap-1 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-[12px] font-bold text-text-main hover:border-primary hover:text-primary transition-all shadow-sm">
            ...
            @empty
                <div class="px-6 py-20 text-center">
                    <div class="size-20 rounded-full bg-gray-50 flex items-center justify-center mx-auto mb-4">
                        <span class="material-symbols-outlined text-gray-300 text-[40px]">group</span>
                    </div>
                    <p class="text-text-muted text-sm font-bold">
                        {{ $activeFilterCount > 0 ? '조건에 맞는 회원이 없습니다.' : '등록된 회원이 없습니다.' }}
                    </p>
                    <p class="mt-2 text-[12px] font-bold text-text-muted">
                        {{ $activeFilterCount > 0 ? '검색 조건을 조정하거나 필터를 초기화해 다시 확인해 주세요.' : '회원가입 데이터가 쌓이면 이곳에서 회원 상태를 관리할 수 있습니다.' }}
                    </p>
                </div>
            @endforelse
        </div>
