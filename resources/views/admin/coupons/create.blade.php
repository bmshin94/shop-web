...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-xl font-black text-text-main">새로운 쿠폰 생성</h2>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <form action="{{ route('admin.coupons.store') }}" method="POST" class="p-6 lg:p-8 space-y-8 pt-4">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="예: 웰컴 할인 쿠폰" 
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" placeholder="예: WELCOME2026" 
                               class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all uppercase">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="쿠폰 사용 조건 등을 입력해 주세요." 
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all">{{ old('description') }}</textarea>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <select name="type" id="type" class="w-full px-4 py-3 pr-10 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 appearance-none outline-none transition-all cursor-pointer !bg-none">
                                <option value="discount" {{ old('type') == 'discount' ? 'selected' : '' }}>할인 쿠폰</option>
                                <option value="shipping" {{ old('type') == 'shipping' ? 'selected' : '' }}>배송비 쿠폰</option>
                            </select>
                            <span class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none group-focus-within:text-primary">expand_more</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <select name="discount_type" id="discount_type" class="w-full px-4 py-3 pr-10 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 appearance-none outline-none transition-all cursor-pointer !bg-none">
                                <option value="fixed" {{ old('discount_type') == 'fixed' ? 'selected' : '' }}>금액 할인 (원)</option>
                                <option value="percent" {{ old('discount_type') == 'percent' ? 'selected' : '' }}>비율 할인 (%)</option>
                            </select>
                            <span class="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none group-focus-within:text-primary">expand_more</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', 0) }}" 
                                   class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all">
                    ...
                    <div class="space-y-2" id="max-discount-wrapper">
                        ...
                        <div class="relative group">
                            <input type="number" name="max_discount_amount" id="max_discount_amount" value="{{ old('max_discount_amount') }}" placeholder="제한 없음"
                                   class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all">
                            <span class="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-black text-text-muted group-focus-within:text-primary transition-colors">원</span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" name="starts_at" id="starts_at" value="{{ old('starts_at') }}" placeholder="시작일 선택"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" name="ends_at" id="ends_at" value="{{ old('ends_at') }}" placeholder="종료일 선택"
                                   class="datepicker w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-semibold focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all">
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        const $maxDiscountWrapper = $('#max-discount-wrapper');

        /**
         * 할인 방식에 따른 UI 상태 업데이트
         */
            const isPercent = $discountTypeSelect.val() === 'percent';
            $discountUnit.text(isPercent ? '%' : '원');
            
            if (isPercent) {
                $maxDiscountWrapper.css({ 'opacity': '1', 'pointer-events': 'auto' });
            } else {
                $maxDiscountWrapper.css({ 'opacity': '0.5', 'pointer-events': 'none' });
            }
        }

        $discountTypeSelect.on('change', updateUI);
        updateUI(); // 초기화 실행
    });
</script>
@endpush
