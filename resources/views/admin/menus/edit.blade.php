...
@section('page_title', '메뉴 정보 수정')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="flex items-center justify-between mb-8">
        <h3 class="text-2xl font-extrabold text-text-main">메뉴 정보 수정</h3>
        ...
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST" class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="group_name" value="{{ old('group_name', $menu->group_name) }}" placeholder="예: 쇼핑몰 관리" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $menu->name) }}" required placeholder="예: 상품 관리" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                </div>
                <div>
                    <label class="block text-xs font-black text-text-muted uppercase mb-2 ml-1">아이콘 (Material Icon)</label>
                    <input type="text" name="icon" value="{{ old('icon', $menu->icon) }}" placeholder="예: inventory_2" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                </div>
            </div>

            <div>
                <label class="block text-xs font-black text-text-muted uppercase mb-2 ml-1">라우트명 (Route Name)</label>
                <input type="text" name="route" value="{{ old('route', $menu->route) }}" placeholder="예: admin.products.index" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
            </div>

            <div>
                <label class="block text-xs font-black text-text-muted uppercase mb-2 ml-1">권한 체크 키 (Permission Key)</label>
                <input type="text" name="permission_key" value="{{ old('permission_key', $menu->permission_key) }}" placeholder="예: products" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-xs font-black text-text-muted uppercase mb-2 ml-1">정렬 순서</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $menu->sort_order) }}" required class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none transition-all">
                </div>
                <div>
                    ...
                    <div class="flex gap-4">
                        <label class="flex-1 flex items-center justify-center gap-2 p-4 bg-gray-50 rounded-2xl border border-gray-200 cursor-pointer hover:bg-white hover:border-primary transition-all group">
                            <input type="radio" name="is_active" value="1" {{ old('is_active', $menu->is_active) == '1' ? 'checked' : '' }} class="w-4 h-4 text-primary border-gray-300 focus:ring-primary/20">
                        ...
                        <label class="flex-1 flex items-center justify-center gap-2 p-4 bg-gray-50 rounded-2xl border border-gray-200 cursor-pointer hover:bg-white hover:border-red-500 transition-all group">
                            <input type="radio" name="is_active" value="0" {{ old('is_active', $menu->is_active) == '0' ? 'checked' : '' }} class="w-4 h-4 text-red-500 border-gray-300 focus:ring-red-500/20">
